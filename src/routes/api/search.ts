...
import { z } from "zod";
const schema = z.object({ q: z.string().trim().min(1).max(200), page: z.coerce.number().int().min(1).max(100).default(1), pageSize: z.coerce.number().int().min(1).max(20).default(10), fresh: z.enum(["1"]).optional() });
export const Route = createFileRoute("/api/search")({ server: { handlers: { GET: async ({ request }) => {
  ...
  try { const { searchWeb } = await import("@/lib/search.server"); return Response.json(await searchWeb(parsed.data.q, parsed.data.page, parsed.data.pageSize, parsed.data.fresh === "1"), { headers: { "Cache-Control": "private, max-age=30" } }); }
